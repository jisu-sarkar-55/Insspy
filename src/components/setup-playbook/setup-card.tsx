...
export function SetupCard({ playbook, onDelete }: SetupCardProps) {
  ...
  return (
    <>
      <div
        className="group relative overflow-hidden rounded-lg border transition-all hover:scale-[1.01] hover:shadow-lg"
        style={{
          background: "var(--surface-card)",
          borderColor: "var(--border-subtle)",
        }}
      >
        <Link href={`/dashboard/setup-playbook/${playbook.id}`} className="block">
          {hasScreenshot && (
        ...
        <div className="p-4">
          <div className="mb-2 flex items-start justify-between gap-2">
            <Link href={`/dashboard/setup-playbook/${playbook.id}`} className="flex-1 min-w-0">
            ...
            <button
              ...
              className="shrink-0 rounded-md p-1.5 opacity-0 transition-all group-hover:opacity-100 hover:opacity-100"
              style={{ color: "var(--text-muted)" }}
            >
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete "${playbook.name}"? This will not delete linked trades.`}
